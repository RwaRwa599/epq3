...
def classify_mark_nonverbal(
    ...
    *,
    ...
    # Paddle saw no mark text. Trust a clear density slash/fill; otherwise empty.
    if density_pred.is_marked and density_pred.source in {"slash", "filled"}:
        return density_labeled

    if density_pred.is_marked and density_pred.ink_density >= 0.20:
        return density_labeled

    return MarkPrediction(
        field_id=field_id,
        is_marked=False,
        confidence=round(max(density_pred.confidence, 0.7), 3),
        ink_density=density_pred.ink_density,
        needs_hitl=density_pred.needs_hitl,
        source="paddle",
    )
