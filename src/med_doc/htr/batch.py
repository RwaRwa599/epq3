...
from med_doc.htr.verbal import attach_kg_priors_batch, recognize_fields
...
def process_block1_document(
    ...
    *,
    ...
    if run_vb:
        ...
        hw = attach_kg_priors_batch(hw, kg, ticked_ids=ticked, expected_tubes=expected)

    return hypotheses_from_parts(doc.doc_id, marks, hw, implied=implied)
...
def process_from_block1(
    ...
    *,
    ...
    Does not require Block 2's per-sheet batch. Pass `kg=KnowledgeGraph.load()` to
    import the frozen Block 2 catalogue for optional `assume()` on write-ins.
    """
    if kg is None:
        try:
    ...
    for doc in iter_block1_documents(base_in_dir):
        ...
        hyp = process_block1_document(doc, kg=kg, backend=backend, mode=mode)
        prediction = prediction_from_hypotheses(hyp, kg=kg)
