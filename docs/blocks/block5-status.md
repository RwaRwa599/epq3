...
5. **Registration sanity (independent of CV)** — if ≥40 ordered tests, or ticked ids are ≥40% of checkbox fields on a full sheet (`n_checkbox ≥ 80`), or **crop_retry_rate ≥ 40%** on a full sheet, or ticks exist but every tube crop is empty (`{}` counts as empty) / implausible, set `needs_review=True`, `registration_failure_suspected=True`. Do **not** use ticked/ordered (that is 100% whenever the lists match). 17 ticks on a 138-box sheet must not trip the gate. A 98% 1c retry with a bare `observed_tubes: {}` must trip it.
